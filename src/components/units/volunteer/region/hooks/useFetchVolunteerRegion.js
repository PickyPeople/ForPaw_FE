...
export default function useFetchVolunteerRegion() {
  ...
  const [volunteerRegionInfos, setVolunteerRegionInfos] = useState(
    volunteerInfos.localGroups
  ); //초기 지역모임 값들만 가져온다.

  useEffect(() => { //초기값을 불러와주는 기능
    if (typeof window !== 'undefined') {
      const savedPageNumber = localStorage.getItem('pageNumber_region');
      setPageNumber(savedPageNumber ? parseInt(savedPageNumber) : 0);

      const savedVolunteerData = localStorage.getItem('volunteerRegionData');
      if (savedPageNumber) {
        setVolunteerRegionInfos(JSON.parse(savedVolunteerData));
      }
    }
  }, []);
