...
export default function useFetchVolunteerNewGrouop() {
  ...
  const [volunteerNewGroupInfos, setVolunteerNewGroup] = useState(
    volunteerInfos.newGroups
  );

  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedPageNumber = localStorage.getItem("pageNumber_newGroup");
      setPageNumber(savedPageNumber ? parseInt(savedPageNumber) : 0);

      const savedVolunteerData = localStorage.getItem("volunteerNewGroupData");
      if (savedPageNumber) {
        setVolunteerNewGroup(JSON.parse(savedVolunteerData));
      }
    }
  }, []);
